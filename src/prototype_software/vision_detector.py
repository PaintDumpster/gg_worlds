...
class QRCodeDetector(visionDetector):
    def __init__(self, camera_input=None):
        ...
        # Simple persistence variables
        self.last_qr_content = None
        self.last_qr_points = None
    
    def detect_qr(self):
        """Detects QR codes in the camera input and maintains data until a new code appears"""
        ...
        try:
            ...
            # Update only if we have a new valid QR code with different content
            if points is not None and qr_content and qr_content != self.last_qr_content:
                self.last_qr_content = qr_content
                self.last_qr_points = points.astype(np.int32)
            
            # Always draw the last detected QR code if we have one
